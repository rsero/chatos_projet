	...
	private void processIn(ClientChatos client, SelectionKey key) throws IOException {
		...
		for (var cpt =0 ; ; cpt++) {
			...
			if (isPrivateConnection && cpt == 0) {
				System.out.println("je suis privée");
				status = httpReader.process(bbin, key);
				System.out.println("je suis privée1");
			} else {
			...
			case DONE:
				if (isPrivateConnection && cpt == 0) {
					System.out.println("je suis privée2");
					Data value = httpReader.get();
					System.out.println("je suis privée3");
					value.decode(client, key);
					System.out.println("je suis privée4");
					httpReader.reset();
				} else {
