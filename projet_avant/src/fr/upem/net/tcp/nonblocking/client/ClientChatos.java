...
import fr.upem.net.tcp.nonblocking.server.data.*;
...
public class ClientChatos {
	...
	private Optional<ByteBuffer> parseRefusePrivateConnection(ByteBuffer req, String data) throws IOException {
		if (data.isEmpty()) {
	...
	private Optional<ByteBuffer> parseAcceptPrivateConnection(ByteBuffer req, String data) throws IOException {
		if (data.isEmpty()) {
	...
	private Optional<ByteBuffer> disconnectPrivateClient(ByteBuffer req, Login loginTarget) throws IOException {
		...
		var disconnectRequest = new DisconnectRequest(connectId);
	...
	public void deleteRequestConnection(Login loginTarget) {
		...
		synchronized (lock) {
			System.out.println(">>>" + hashLoginFile.keySet().size());
			hashLoginFile.remove(loginTarget);
			System.out.println("<<<" + hashLoginFile.keySet().size());
		}
	}
